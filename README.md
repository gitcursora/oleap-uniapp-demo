...
- 转写页已接入七牛直传 + 火山引擎 ASR 真实转写，但后续的摘要、关键词等 AI 处理仍需学生自行扩展。
...
```text
1. 获取凭证：调用后端 /api/qiniu/token 获取七牛上传凭证
2. 上传文件：使用 uni.uploadFile 直传七牛 OSS，拿到公网音频 URL
3. 转写音频：调用后端 /api/asr/transcribe 提交火山引擎 ASR 转写任务
```

这个流程的优势是：

- 音频文件不经过后端中转，减轻后端压力
- 七牛 CDN 提供公网访问能力，火山引擎可以直接拉取音频
- 不依赖本机公网 IP，只要七牛域名和火山引擎 API Key 配置正确即可

如果你是学生，最适合在这里做的事情是：

- 理解三步上传转写流程
- 在转写结果基础上做摘要、关键词、待办、脑图
- 扩展成自己的业务场景（会议助手、课堂笔记、采访整理等）
...
耳机
  ...
  -> transcript 页面
  -> 获取七牛上传凭证（后端 /api/qiniu/token）
  -> 直传七牛 OSS（uni.uploadFile）
  -> 调用 ASR 转写（后端 /api/asr/transcribe）
  -> 展示转写结果
```
...
- 讲如何把音频送到后端（七牛直传 + ASR 转写两步流程）
- 讲转写页面的三步流程：获取凭证 -> 上传七牛 -> 调用转写
- 让学生开始做自己的场景
