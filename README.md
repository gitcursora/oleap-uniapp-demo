...
- P3 Android 已推进：实时录音 start/stop、响应解析、OPUS notify 帧切分、丢包统计、`.opusraw`/`.oleapframes` 落盘和录音进度已接入 UTS 平台层；libopus/WAV finalize 仍待下一步集成。
- 默认使用 mock mode，可在没有耳机的情况下跑通页面和 API 调用链。
- Flash 文件协议、iOS 原生 BLE 和 OPUS 解码在后续 Phase 实现。
